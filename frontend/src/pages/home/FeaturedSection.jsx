import React from 'react';
import { Link } from 'react-router-dom'; // Import Link
import FeaturedImg from "../../assets/FeaturedImg.jpg";

const FeaturedSection = () => {
    return (
        <div className='overflow-hidden flex md:flex-row flex-col justify-between items-center sm:my-20 my-4 md:gap-20 gap-12 px-5 lg:px-10'>
            <div className='relative'>
                <div className='absolute top-4 left-5 bg-white text-secondary px-3 py-1 rounded-md uppercase tracking-wider'>Featured Recipe</div>
                <img
                    src={FeaturedImg}
                    alt='Featured image'
                    className='rounded-2xl w-full h-auto md:w-[800px] md:h-[400px]'
                />
            </div>

            <div className='text-start sm:w-1/2'>
                <h2 className='text-2xl font-semibold text-secondary sm:text-5xl sm:leading-relaxed'>Sweet and Sour Tofu</h2>
                <p className='text-xl mt-4 text-[#5c5c5c]'>Indulge in our sweet and sour tofu—where every bite bursts with a tantalizing balance of tangy goodness and rich flavor, leaving your taste buds craving more!</p>
                <div className='lg:mt-0 lg:flex-shrink-0'>
                    <div className='mt-12 inline-flex'>
                        <Link to="/items/66eef57dea07d0f5ab325139"> {/* Updated to Link */}
                            <button className='py-4 px-8 hover:bg-btnColor text-secondary hover:text-white w-full transition ease-in duration-200 text-center text-base font-semibold border border-[#9c702a] focus:outline-none rounded-lg'>
                                View Recipe
                            </button>
                        </Link>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default FeaturedSection;
